...
import "../styles/FacultyApproval.css"; // Import custom CSS for FacultyApproval component
import { useLocation } from "react-router-dom"; // Import useLocation for URL query parameters
import "../styles/Style.css"; // Import global CSS styles
import Swal from "sweetalert2"; // Import SweetAlert2 for user feedback modals

function FacultyApproval() { // Define FacultyApproval functional component
  // Retrieve API configuration from environment variables
  const port = import.meta.env.VITE_API_PORT;
  const ip = import.meta.env.VITE_API_IP;

  // State definitions
  const [faculties, setFaculties] = useState([]); // Store list of faculty records
  const [popupData, setPopupData] = useState(null); // Store data for detailed faculty popup
  const [newUsersCount, setNewUsersCount] = useState(0); // Store count of new users
  const [existingUsersCount, setExistingUsersCount] = useState(0); // Store count of existing users
  const [dataEntriesCount, setDataEntriesCount] = useState(0); // Store count of data entries
  const [approvalStatus, setApprovalStatus] = useState({}); // Track approval/rejection status
  const [rejectingFacultyId, setRejectingFacultyId] = useState(null); // Store ID of faculty being rejected
  const [rejectionRemarks, setRejectionRemarks] = useState(""); // Store rejection remarks
  ...
  // Fetch counts for new users, existing users, and data entries on component mount
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        ...
        // Fetch monthly asset and faculty data
  ...
  // Fetch all faculty records on component mount
  useEffect(() => {
  ...
  const submitRejection = async () => {
    ...
    try {
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: error.response?.data?.message || "Error rejecting faculty.",
      });
    } finally {
      setRejectingFacultyId(null);
    }
