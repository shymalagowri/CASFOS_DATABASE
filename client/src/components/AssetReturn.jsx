 ...
 * - Custom subcategory input when "Others" is selected in the subcategory dropdown.
 * 
...
const AssetReturn = () => {
  ...
  // Fetch store items when asset type, category, or subcategory changes
  useEffect(() => {
    if (assetType && assetCategory) {
      const fetchStoreItems = async () => {
        try {
          ...
          setIssuedLocations([]);
          setIssuedIds([]);
          setReturnItem((prev) => ({ ...prev, location: "", issuedQuantity: 0, returnIds: [] }));
        } catch (error) {
  ...
  }, [assetType, assetCategory, subCategory, customSubCategory, ip, port]);
  ...
  }, [assetType, assetCategory, selectedItem, subCategory, customSubCategory, ip, port]);
  ...
  }, [assetType, assetCategory, selectedItem, returnItem.location, subCategory, customSubCategory, ip, port]);
  ...
  const handleSubmitReturn = async () => {
    ...
    if (subCategory === "Others" && !customSubCategory) {
      Swal.fire({
        icon: "warning",
        title: "Warning",
        text: "Please enter a custom subcategory!",
      });
      return;
    }
