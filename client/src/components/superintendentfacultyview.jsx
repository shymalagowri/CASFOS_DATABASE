...
const SuperintendentFacultyView = () => {
    ...
    const fetchUnverifiedFaculties = async () => {
      try {
        ...
        if (filteredData.length > 0) {
          ...
          setTableData([]);
          setMessage("No unverified faculty records found matching the criteria.");
        }
      } catch (error) {
        setTableData([]);
        setMessage("Error fetching unverified faculties.");
        console.error(error);
      }
    };
  ...
  const handleVerifyFaculty = async (facultyId) => {
    try {
      ...
      if (response.data.success) {
        ...
        setMessage("Faculty verified successfully!");
      }
    } catch (error) {
      setMessage("Error verifying faculty.");
      console.error(error);
    }
  };
  ...
  const renderPopupContent = (data) => {
    ...
    // Log the raw data to verify the field names
    console.log("Raw Faculty Data:", data);
  
    const allFields = Object.entries(data)
      .filter(([key]) => key.toLowerCase() !== "_id" && key.toLowerCase() !== "conduct") // Case-insensitive check
      .map(([key, value]) => (
        <tr key={key}>
  ...
  const popupStyles = {
    ...
    popupContent: {
      ...
      borderRadius: "10px",
      width: "90%",
      maxWidth: "800px", // Increased width for more fields
      maxHeight: "80vh",
      overflowY: "auto",
      position: "relative",
    },
...
export default SuperintendentFacultyView;