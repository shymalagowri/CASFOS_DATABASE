/**
 * Overview:
 * This is a React component named `AssetView` designed for a storekeeper dashboard in an asset management system.
...
import React, { act, useEffect, useState } from "react"; // React core, act for testing, useEffect for side effects, useState for state management
import { useLocation } from "react-router-dom"; // Hook to access URL query parameters
import "../styles/Style.css"; // General styles for the application
import "../styles/ViewAsset.css"; // Specific styles for the AssetView component
import axios from "axios"; // Library for making HTTP requests to the backend API
import jsPDF from "jspdf"; // Library for generating PDF documents
import "jspdf-autotable"; // Plugin for creating tables in jsPDF
import * as XLSX from "xlsx"; // Library for generating Excel files

// Define the AssetView functional component
const AssetView = () => {
  // Environment variables for API configuration
  const port = import.meta.env.VITE_API_PORT; // Backend API port from environment variables
  const ip = import.meta.env.VITE_API_IP; // Backend API IP address from environment variables

  // Access URL query parameters using useLocation
  const location = useLocation();
  const queryParams = new URLSearchParams(location.search);
  const username = queryParams.get("username") || "Guest"; // Extract username from URL or default to "Guest"

  // State management for various component features
  const [zoomedImage, setZoomedImage] = useState(null); // State for handling zoomed image display
  const [sortConfig, setSortConfig] = useState({ key: null, direction: "asc" }); // State for table sorting configuration
  const [buildingCondemnationData, setBuildingCondemnationData] = useState([]); // State for building condemnation data
  const [activeTab, setActiveTab] = useState("purchase"); // State for tracking the active tab (e.g., purchase, storeIssue)
  const [buildingMaintenanceData, setBuildingMaintenanceData] = useState([]); // State for building maintenance data
  const [purchaseFilters, setPurchaseFilters] = useState({ // State for purchase tab filters
    assetType: "",
    customAssetType: "",
    assetCategory: "",
    customAssetCategory: "",
  ...
  const [storeIssueFilters, setStoreIssueFilters] = useState({ // State for store issue tab filters
    assetType: "",
    customAssetType: "",
    assetCategory: "",
    customAssetCategory: "",
  ...
  const [serviceReturnFilters, setServiceReturnFilters] = useState({ // State for service/return tab filters
    assetType: "",
    customAssetType: "",
    assetCategory: "",
    customAssetCategory: "",
  ...
  const [disposalFilters, setDisposalFilters] = useState({ // State for disposal tab filters
    assetType: "",
    customAssetType: "",
    assetCategory: "",
    customAssetCategory: "",
  ...
  const methodOfDisposalOptions = ["", "Sold", "Auctioned", "Destroyed", "Others"]; // Options for disposal method dropdown
  const [tableData, setTableData] = useState([]); // State for table data fetched from API
  const [message, setMessage] = useState(""); // State for displaying messages (e.g., "No records found")
  const [selectedDetails, setSelectedDetails] = useState(null); // State for selected row details
  const [deadStockFilters, setDeadStockFilters] = useState({ // State for dead stock tab filters
    assetType: "",
    customAssetType: "",
    assetCategory: "",
    customAssetCategory: "",
  ...
  const assetTypeOptions = ["", "Permanent", "Consumable"]; // Asset type options
  const permanentAssetOptions = [ // Options for permanent asset categories
    "",
    "Furniture",
    "Vehicle",
    "Building",
  ...
  const consumableAssetOptions = [ // Options for consumable asset categories
    "",
    "Stationery",
    "IT",
    "Electrical",
  ...
  const sourceOptions = ["", "GEM", "Local", "Other"]; // Source of purchase options
  const modeOfPurchaseOptions = ["", "Tender", "Quotation", "Others"]; // Mode of purchase options
  const conditionOptions = ["", "InService", "Serviced", "Returned", "Exchanged"]; // Condition options for service/return
  const locationOptions = [ // Location options for assets
    "store",
    "all_issued",
    "faculty_chamber",
    "officer_quarters",
  ...
  const subCategoryOptions = { // Subcategory options mapped to asset categories
  ...
  const Posters = (key) => {
    ...
    // Sort table data based on the key and direction
    const sortedData = [...tableData].sort((a, b) => {
      let aValue = a[key];
      let bValue = b[key];

      // Handle date fields by converting to Date objects
      ...
      // Handle numeric fields by parsing to float
      ...
      // Handle arrays (e.g., itemIds, issuedIds) by joining into strings
    ...
    setTableData(sortedData); // Update table data with sorted results
  };

  // useEffect hook to fetch data when activeTab or filters change
  useEffect(() => {
    // Async function to fetch data from the backend
    const fetchData = async () => {
      let endpoint = ""; // API endpoint to be determined based on activeTab
      let filters = {}; // Filters to be sent with the API request
      if (["purchase", "storeIssue", "deadStock"].includes(activeTab)) {
        setBuildingMaintenanceData([]); // Clear building maintenance data for non-relevant tabs
        setBuildingCondemnationData([]); // Clear building condemnation data for non-relevant tabs
      }
      // Determine endpoint and filters based on activeTab
      switch (activeTab) {
        case "purchase":
          endpoint = "/api/assets/filterPurchase"; // Endpoint for purchase data
          filters = {
            ...purchaseFilters,
            ...
            // Exclude itemName for Building and Land categories
            ...(purchaseFilters.assetCategory !== "Building" && purchaseFilters.assetCategory !== "Land" ? { itemName: purchaseFilters.itemName } : {}),
          };
          break;
        case "storeIssue":
          endpoint = "/api/assets/filterStoreIssue"; // Endpoint for store issue data
          filters = {
            ...storeIssueFilters,
          ...
          break;
        case "serviceReturn":
          endpoint = "/api/assets/filterServiceReturn"; // Endpoint for service/return data
          filters = {
            ...
            buildingNo: serviceReturnFilters.buildingNo, // Include building number filter
          };
          break;
        case "disposal":
          endpoint = "/api/assets/filterDisposal"; // Endpoint for disposal data
          filters = {
            ...disposalFilters,
          ...
          break;
        case "deadStock":
          // Update dead stock quantities before fetching data
          try {
          ...
          endpoint = "/api/assets/filterDeadStock"; // Endpoint for dead stock data
          filters = {
            ...deadStockFilters,
      ...
      try {
        ...
        if (activeTab === "serviceReturn") {
          setTableData(response.data.serviceReturn || []); // Set service/return data
          setBuildingMaintenanceData(response.data.buildingMaintenance || []); // Set building maintenance data
          setMessage(
            response.data.serviceReturn.length === 0 && response.data.buildingMaintenance.length === 0
              ? "No matching records found."
              : ""
          ); // Set message if no records found
        } else if (activeTab === "disposal") {
          setTableData(response.data.disposal || []); // Set disposal data
          setBuildingCondemnationData(response.data.buildingCondemnation || []); // Set building condemnation data
          setMessage(
          ...
          setTableData(response.data); // Set table data for other tabs
          setMessage(response.data.length === 0 ? "No matching records found." : ""); // Set message if no records found
        }
      } catch (error) {
        setTableData([]); // Clear table data on error
        setBuildingMaintenanceData([]); // Clear building maintenance data
        setBuildingCondemnationData([]); // Clear building condemnation data
        setMessage("Error fetching data."); // Set error message
        console.error(error); // Log error to console
      }
    };

    fetchData(); // Call fetchData when useEffect runs
  }, [activeTab, purchaseFilters, storeIssueFilters, serviceReturnFilters, disposalFilters, deadStockFilters]); // Dependencies for useEffect

  // Function to handle filter changes for different tabs
  const handleFilterChange = (filterSetter) => (field, value) => {
    filterSetter((prev) => ({
      ...prev,
      [field]: value,
      // Reset custom fields if "Others" is not selected
  ...
  // Function to clear filters for the active tab
  const handleClearFilter = () => {
    switch (activeTab) {
      case "purchase":
        setPurchaseFilters({ // Reset purchase filters
          assetType: "",
          customAssetType: "",
          assetCategory: "",
          customAssetCategory: "",
        ...
        break;
      case "storeIssue":
        setStoreIssueFilters({ // Reset store issue filters
          assetType: "",
          customAssetType: "",
          assetCategory: "",
          customAssetCategory: "",
        ...
        break;
      case "serviceReturn":
        setServiceReturnFilters({ // Reset service/return filters
          assetType: "",
          customAssetType: "",
          assetCategory: "",
          customAssetCategory: "",
        ...
        break;
      case "disposal":
        setDisposalFilters({ // Reset disposal filters
          assetType: "",
          customAssetType: "",
          assetCategory: "",
          customAssetCategory: "",
        ...
        break;
      case "deadStock":
        setDeadStockFilters({ // Reset dead stock filters
          assetType: "",
          customAssetType: "",
          assetCategory: "",
          customAssetCategory: "",
  ...
  // Function to convert an image URL to base64 for PDF generation
  const getBase64ImageFromUrl = (imageUrl) => {
    return new Promise((resolve, reject) => {
      const img = new Image();
      img.setAttribute("crossOrigin", "anonymous"); // Handle CORS for external images
  ...
  // Function to generate a PDF report based on the active tab
  const generatePDF = async () => {
    const pdf = new jsPDF("l", "mm", "a3"); // Create a new PDF in landscape A3 size
    const pageWidth = pdf.internal.pageSize.getWidth(); // Get page width
    const margin = { left: 10, right: 10 }; // Define margins

    let logoBase64; // Variable for logo image in base64
    try {
      logoBase64 = await getBase64ImageFromUrl("images/CASFOS-Coimbatore.jpg"); // Load logo image
    } catch (error) {
      console.error("Error loading logo image", error);
    }

    const logoWidth = 50; // Logo dimensions
    const logoHeight = 50;
    const logoX = margin.left;
    const logoY = 10;

    if (logoBase64) {
      pdf.addImage(logoBase64, "PNG", logoX, logoY, logoWidth, logoHeight); // Add logo to PDF
    }

    const titleX = pageWidth / 2; // Center title
    const titleY = logoY + logoHeight / 2;

    pdf.setFontSize(30); // Set font size for title
    pdf.setFont("helvetica", "bold");
    pdf.text("Central Academy for State Forest Service", titleX, titleY, { align: "center" }); // Add title

    const currentDateTime = new Date(); // Get current date and time
    const dateString = currentDateTime.toLocaleDateString();
    const timeString = currentDateTime.toLocaleTimeString();

    pdf.setFontSize(17); // Set font size for date/time
    pdf.text(`Date: ${dateString}`, pageWidth - 60, logoY + 20); // Add date
    pdf.text(`Time: ${timeString}`, pageWidth - 60, logoY + 30); // Add time

    const assetReportY = logoY + logoHeight + 20; // Position for report title
    pdf.setFontSize(27);
    pdf.text(
    ...
    if (activeTab === "purchase") {
      ...
      if (purchaseFilters.assetCategory === "Building") {
        ...
        tableRows = tableData.map((row) => { // Map data to rows
          const details = [
            `Type: ${row.type || "N/A"}`,
        ...
        tableColumn = [ // Columns for land assets
          "Asset Type",
          "Asset Category",
          "Sub Category",
          "Date of Possession",
        ...
        tableColumn = [ // Columns for other assets
          "Asset Type",
          "Asset Category",
          "Sub Category",
          "Item Name",
      ...
      pdf.autoTable({ // Generate table in PDF
        startY,
        head: [tableColumn],
        body: tableRows,
        theme: "grid",
      ...
      startY = pdf.lastAutoTable.finalY; // Update startY for next content
      ...
      if (totalCost.serviceCost || totalCost.maintenanceCost) {
        ...
        if (totalCost.serviceCost) {
          pdf.text(`Total Service Cost: ₹${totalCost.serviceCost}`, pageWidth - 60, startY); // Add total service cost
          startY += 10;
        }
        if (totalCost.maintenanceCost) {
          pdf.text(`Total Maintenance Cost: ₹${totalCost.maintenanceCost}`, pageWidth - 60, startY); // Add total maintenance cost
          startY += 10;
        }
      }
      ...
      if (serviceReturnFilters.assetCategory === "Building" && buildingMaintenanceData?.length > 0) {
        pdf.addPage(); // Add new page for building maintenance data
        pdf.setFontSize(27);
        pdf.text("Building Maintenance Records", pageWidth / 2, 20, { align: "center" }); // Add title
        startY = totalCost.maintenanceCost ? 40 : 30;
        if (totalCost.maintenanceCost) {
          pdf.setFontSize(14);
          pdf.text(`Total Maintenance Cost: ₹${totalCost.maintenanceCost}`, pageWidth - 60, 30); // Add total maintenance cost
        }
      ...
      if (totalCost.serviceCost || totalCost.demolitionEstimate) {
        ...
        pdf.setTextColor(0, 0, 0);
        if (totalCost.serviceCost) {
          pdf.text(`Total Disposal Value: ₹${totalCost.serviceCost}`, pageWidth - 60, startY); // Add total disposal value
          startY += 10;
        }
        if (totalCost.demolitionEstimate) {
          pdf.text(`Total Demolition Estimate: ₹${totalCost.demolitionEstimate}`, pageWidth - 60, startY); // Add total demolition estimate
          startY += 10;
        }
      }
      ...
      const tableRows = tableData.map((row) => [
        ...
        row.subCategory || "N/A",
        row.itemName || "N/A",
        row.itemIds?.length > 0 ? `${row.itemIds.join(", ")} (${row.quantity || "N/A"})` : row.quantity || "N/A",
      ...
      if (disposalFilters.assetCategory === "Building" && buildingCondemnationData?.length > 0) {
        pdf.addPage(); // Add new page for building condemnation data
        pdf.setFontSize(27);
        pdf.text("Building Condemnation Records", pageWidth / 2, 20, { align: "center" }); // Add title
        startY = totalCost.demolitionEstimate ? 40 : 30;
        if (totalCost.demolitionEstimate) {
          pdf.setFontSize(14);
          pdf.text(`Total Demolition Estimate: ₹${totalCost.demolitionEstimate}`, pageWidth - 60, 30); // Add total demolition estimate
        }
    ...
    pdf.save(`${activeTab === "deadStock" ? "dead_stock_register" : activeTab}_asset_report.pdf`); // Save the PDF
  };

  // Function to generate an Excel report based on the active tab
  const generateExcel = () => {
    const wb = XLSX.utils.book_new(); // Create a new Excel workbook

    // Note: The provided code snippet for the purchase tab is incomplete, so commenting based on the pattern
    if (activeTab === "purchase") {
      if (purchaseFilters.assetCategory === "Building") {
        wsData.push([ // Headers for building assets
          "Asset Type",
          "Asset Category",
          "Sub Category",
          "Building No",
        ...
        tableData.forEach((row) => { // Map data to rows
          wsData.push([
            row.assetType,
            row.assetCategory,
      ...
      const ws = XLSX.utils.aoa_to_sheet([headers, ...data]); // Create worksheet
      ws["!cols"] = headers.map((header, index) => { // Set column widths
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Store Assets"); // Add worksheet to workbook
    } else if (activeTab === "storeIssue" && storeIssueFilters.location !== "store") {
      const headers = [
        "Asset Type",
        "Asset Category",
      ...
      const wsData = [];
      if (totalCost.serviceCost) {
        wsData.push(["Total Service Cost", `₹${totalCost.serviceCost}`]); // Add total service cost
      }
      if (totalCost.maintenanceCost) {
        wsData.push(["Total Maintenance Cost", `₹${totalCost.maintenanceCost}`]); // Add total maintenance cost
      }
      if (wsData.length > 0) {
        wsData.push([]); // Add empty row for spacing
      }
      wsData.push(headers, ...data); // Add headers and data
      ...
      if (serviceReturnFilters.assetCategory === "Building" && buildingMaintenanceData.length > 0) {
        ...
        if (totalCost.maintenanceCost) {
          ...
          buildingWsData.push([]); // Add empty row for spacing
        }
      ...
      const data = tableData.map((row) => [
        ...
        row.subCategory,
        row.itemName,
        row.itemIds?.length > 0 ? `${row.itemIds.join(", ")} (${row.quantity || "N/A"})` : row.quantity || "",
        row.purchaseValue,
        row.bookValue,
      ...
      const wsData = [];
      if (totalCost.serviceCost) {
        wsData.push(["Total Disposal Value", `₹${totalCost.serviceCost}`]); // Add total disposal value
      }
      if (totalCost.demolitionEstimate) {
        wsData.push(["Total Demolition Estimate", `₹${totalCost.demolitionEstimate}`]); // Add total demolition estimate
      }
      if (wsData.length > 0) {
        wsData.push([]); // Add empty row for spacing
      }
    ...
    XLSX.writeFile(wb, `${activeTab === "deadStock" ? "dead_stock_register" : activeTab}_assets.xlsx`); // Save the Excel file
  };

  // Function to show details of a selected row
  const showDetails = (row) => {
    setSelectedDetails(row); // Set selected row details
  };
  ...
  // Function to calculate total costs for different tabs
  const calculateTotalCost = () => {
    ...
    if (activeTab === "purchase") {
      purchaseCost = tableData.reduce((sum, row) => { // Calculate total purchase cost
      ...
      if (serviceReturnFilters.condition !== "InService" && serviceReturnFilters.condition !== "Exchanged") {
        serviceCost = tableData.reduce((sum, row) => sum + (parseFloat(row.serviceAmount) || 0), 0); // Calculate total service cost
      }
      if (serviceReturnFilters.assetCategory === "Building" && buildingMaintenanceData?.length > 0) {
        maintenanceCost = buildingMaintenanceData.reduce((sum, row) => { // Calculate total maintenance cost
          return sum + (parseFloat(row.cost) || 0);
        }, 0);
      }
    } else if (activeTab === "disposal") {
      disposalValue = tableData.reduce((sum, row) => sum + (parseFloat(row.disposalValue) || 0), 0); // Calculate total disposal value
      if (disposalFilters.assetCategory === "Building" && buildingCondemnationData?.length > 0) {
        demolitionEstimate = buildingCondemnationData.reduce((sum, row) => { // Calculate total demolition estimate
          return sum + (parseFloat(row.demolitionEstimate) || 0);
        }, 0);
      }
    } else if (activeTab === "storeIssue" && storeIssueFilters.location !== "store") {
      storeIssueValue = tableData.reduce((sum, row) => { // Calculate total store issue value
  ...
  const totalCost = calculateTotalCost(); // Calculate and store total costs
    ...
    <div className="admin-asset-view">
      ...
      <section id="sidebar">
          ...
          <ul className="side-menu top"> {/* List of primary navigation links */}
            ...
            <li className="active"> {/* Asset View link */}
...
const styles = {
  ...
  popupContent: {
    ...
    display: "flex",
    flexDirection: "column",
    overflowY: "auto", // Allow scrolling for the entire popup
  },
  tableContainer: {
    flex: "1 1 auto", // Allow table container to take available space
    overflowY: "auto", // Scrollable table content
    width: "100%",
    marginBottom: "20px", // Space for the button
  },
