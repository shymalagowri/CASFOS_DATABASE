/**
 * Overview:
 * The `AssetIssue` component is a React functional component designed for issuing assets in an Asset Management System.
 * It allows users to issue Permanent or Consumable assets by selecting asset details, specifying quantities, and assigning them
 * to locations or individuals. The component supports editing rejected issues, fetching data from a backend API, generating PDF
 * receipts, and handling acknowledgment of issued assets with signed receipt uploads. It uses SweetAlert2 for alerts and integrates
 * with React Router for navigation and Axios for API requests.
 * Key features include:
 * - Dynamic dropdowns for asset type, category, subcategory, and item selection.
 * - Support for custom subcategory input when "Others" is selected.
 * - Handling of item IDs for Permanent assets with a "Select All" option.
 * - Form validation and error handling.
 * - PDF receipt generation using jsPDF.
 * - Acknowledgment tab for uploading signed receipts.
 * - Styling with inline styles and an external CSS file.
 * - Sidebar navigation for storekeeper functionalities.
 * - Support for editing previously rejected issues using a `rejectedId` query parameter.
 * 
 * Dependencies: React, react-helmet, axios, react-router-dom, sweetalert2, jspdf, react-icons
...
const AssetIssue = () => {
    ...
    if (rejectedId) {
      const fetchRejectedAsset = async () => {
        try {
          ...
          setSubCategory(rejectedAsset.subCategory && subCategoryOptions[rejectedAsset.assetCategory]?.includes(rejectedAsset.subCategory) ? rejectedAsset.subCategory : "Others");
          setCustomSubCategory(rejectedAsset.subCategory && !subCategoryOptions[rejectedAsset.assetCategory]?.includes(rejectedAsset.subCategory) ? rejectedAsset.subCategory : "");
  ...
  }, [rejectedId, serverBaseUrl]);

  // Effect to fetch store items based on asset type, category, and subcategory
  useEffect(() => {
      ...
      const fetchStoreItems = async () => {
        try {
          const response = await axios.post(`${serverBaseUrl}/api/assets/getStoreItems`, {
            assetType,
            assetCategory,
            subCategory: subCategory === "Others" ? customSubCategory : subCategory || undefined,
          });
  ...
  }, [assetType, assetCategory, subCategory, customSubCategory, serverBaseUrl]);

  // Effect to reset subCategory and customSubCategory when assetCategory changes
  useEffect(() => {
    setSubCategory("");
  ...
  useEffect(() => {
    if (assetType && assetCategory && selectedItem) {
      const [itemName, subCategoryFromItem, itemDescription] = selectedItem.split(" - ");
      const fetchStock = async () => {
        try {
          const stockResponse = await axios.post(`${serverBaseUrl}/api/assets/checkInStock`, {
            ...
            assetCategory,
            itemName,
            subCategory: subCategory === "Others" ? customSubCategory : subCategory || subCategoryFromItem,
            itemDescription,
          });
  ...
  }, [assetType, assetCategory, selectedItem, subCategory, customSubCategory, serverBaseUrl]);
  ...
  const handleSubmitIssue = async () => {
    ...
    if (subCategory === "Others" && !customSubCategory) {
      await Swal.fire({ icon: "warning", title: "Warning", text: "Please enter a custom subcategory!" });
      return;
    }
            ...
            {activeTab === "issue" && (
              <div style={styles.formContainer}>
                ...
                <div style={styles.formRow}>
                  ...
                  {assetCategory && subCategoryOptions[assetCategory] && (
                    <div style={styles.inputGroup}>
                      ...
                      >
                        ...
                        {subCategoryOptions[assetCategory].map((option) => (
                          <option key={option} value={option}>
                            {option}
                          </option>
                        ))}
