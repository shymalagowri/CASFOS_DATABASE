...
const deleteAutoSavedFaculty = async (req, res) => {
  ...
  try {
    ...
    //(`Auto-save data deleted for staff ID: ${staffid}`);
...
const deleteFaculty = async (req, res) => {
  ...
  try {
    ...
    if (!deletedFaculty) {
      ...
      //(`Faculty deleted from ConfirmedFaculty with ID: ${id}`);
      return res.status(200).send({
        success: true,
    ...
    //(`Faculty deleted from Faculty with ID: ${id}`);
    res.status(200).send({
      success: true,
